...
| `setup_deck`    | Build the liquid handler for the chosen backend and, for the liquid handler family, place a tip rack and a 96-well plate. Call this first. `home=true` homes a real STAR (motion; deck must be clear). |
...
| `run_pcr_enrichment_round1` | Run a validated targeted PCR PCR1 master-mix protocol by importing and executing the operator's existing starlab script (not a reimplementation). `chatterbox` dry-runs; `star` requires `confirm=true` (human-gated). See below. |
