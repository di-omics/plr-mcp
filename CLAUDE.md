...
4. **Real protocol runs are human-gated.** `plr_run_pcr_enrichment_round1` on `star`
...
- `plr_run_pcr_enrichment_round1` — runs the operator's **validated** starlab PCR1 script
  (not a reimplementation); `star` needs `confirm=true`.

Codegen — writes files, no hardware:

- `plr_generate_analysis_pipeline` — tool-agnostic, UMI-aware single-cell RNA-seq workflow hooks.
