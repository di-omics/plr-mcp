...
# Back-compat: PLR_MCP_SIMULATE=0 used to mean "real hardware" (liquid handler).
...
async def setup_deck(
    ...
    backend: 'chatterbox' (simulation, no hardware), 'star' (liquid handler),
...
@mcp.tool(
    ...
    annotations=ToolAnnotations(
        title="Run pcr_enrichment PCR1 (validated)",
        readOnlyHint=False,
        destructiveHint=True,
        idempotentHint=False,
        openWorldHint=True,
...
) -> schemas.PCR enrichmentPcr1Result:
