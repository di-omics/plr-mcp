...
liquid handler backend for liquid handling; instrument hardware backends are
...
class Lab:
    ...
    def _require_plate(self):
        if self.plate is None or self.tips is None:
            raise LabNotReady(
                "No liquid handler labware loaded for this backend. Tip and plate "
    ...
    def _make_lh(self):
        """Return (liquid_handler_backend, deck, load_liquid_handler_labware).

        Only the liquid handler family (chatterbox, star) auto-loads the tip rack and
        ...
        if self.backend == "star":
            # Real liquid handler. Correct API for 0.2.1; needs the instrument
    ...
    async def setup_deck(self, tip_rail: int = 1, plate_rail: int = 10) -> dict:
        ...
        liquid handler family, lay out a tip rack and a 96-well plate.
        ...
        if load_labware:
            ...
            notes.append(
                f"{self.backend} uses vendor-specific labware; liquid handler tip and "
    ...
    async def _ensure_heater_shaker(self):
        ...
        if self.simulate:
            ...
            raise LabNotReady(
                ...
                "(for example a heater-shaker) into Lab._ensure_heater_shaker."
            )
